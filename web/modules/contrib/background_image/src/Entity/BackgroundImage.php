...
class BackgroundImage extends ContentEntityBase implements BackgroundImageInterface {
  ...
  public function getCssUri() {
    ...
    return "$default_scheme://background_image/css/{$this->id()}/$default_scheme/{$this->language()->getId()}/{$this->getImageHash()}$min.css";
  }
  ...
  public function getTargetWebform($type = NULL, $target = NULL) {
    ...
    if ($target && $type === self::TYPE_ENTITY) {
      list($entity_type, $entity_type_id) = explode(':', $target);
  ...
  public function label($link = FALSE) {
    ...
    else if ($webform = $this->getTargetWebform()) {
      return $this->t('Webform (@entity_id): @entity_label', [
        '@entity_label' => $link && $webform->hasLinkTemplate('edit-form') ? $webform->toLink(NULL, 'edit-form')->toString() : $webform->label(),
  ...
  public function save() {
    ...
    // Clear the generated background image css file on update.
    if ($status === SAVED_UPDATED) {
      $this->getBackgroundImageManager()->cacheFlush($this);
    }

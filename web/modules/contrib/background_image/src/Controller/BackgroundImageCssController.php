...
class BackgroundImageCssController extends FileDownloadController {
  ...
  public function __construct(StreamWrapperManagerInterface $streamWrapperManager, BackgroundImageManagerInterface $background_image_manager, FileSystemInterface $fileSystem, ImageFactory $image_factory, LockBackendInterface $lock, ThemeManagerInterface $theme_manager, CacheFactoryInterface $cache_factory, ModuleExtensionList $moduleExtensionList, TwigEnvironment $twig) {
    ...
    $this->cache = $cache_factory->get('default');
  ...
  public function buildCss(BackgroundImageInterface $background_image, $uri) {
    ...
    try {
      ...
      if (preg_match('/\.min\.css$/', $uri ?? '') &&
        ($css_minifier = $this->backgroundImageManager->getCssMinifier())) {
   ...
   *   The language to load the background image entity in.
   * @param string $file
   ...
   */
  public function deliver(Request $request, BackgroundImageInterface $background_image, $scheme, string $language, $file = NULL) {
    ...
    // Make sure to work with the correct translation.
    if ($background_image->isTranslatable() &&
      $background_image->hasTranslation($language)) {
      $background_image = $background_image->getTranslation($language);
    }

    $uri = "$scheme://background_image/css/{$background_image->id()}/$scheme/{$language}/$file";

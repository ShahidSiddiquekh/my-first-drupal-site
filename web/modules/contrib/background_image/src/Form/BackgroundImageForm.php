...
class BackgroundImageForm implements TrustedCallbackInterface {
  ...
  public function buildTypeSelection() {
    ...
    if (!empty($entity_types)) {
      ...
      unset($this->subform['type']['#options'][BackgroundImageInterface::TYPE_ENTITY]);
    }
    ...
    if (!empty($entity_bundles)) {
      ...
      unset($this->subform['type']['#options'][BackgroundImageInterface::TYPE_ENTITY_BUNDLE]);
    }
    ...
    $this->subform['help'] = [
      '#type' => 'markup',
      '#markup' => $this->t('Enable entity types on the settings tab to be able to select them here.')
    ];
  ...
  public function setFormState(FormStateInterface $form_state) {
    // Unfortunately, due to how forms are processed, the triggering element's
  ...
  public function submit() {
    ...
    try {
      ...
      // clear the css so it can be rebuild.
      if ($status === SAVED_UPDATED) {
        self::getBackgroundImageManager()->cacheFlush($this->backgroundImage);
      }
    }
    catch (\Exception $e) {
      // Intentionally left empty.
      ...
      if (!isset($this->type) || !in_array($this->type, array_keys(BackgroundImage::getTypes()))) {
        ...
        if ($this->type === BackgroundImageInterface::TYPE_ENTITY) {
          ...
          if (!isset($entity_type_id)) {
            ...
            $entity_id = $this->getSubformValue("entity_type_$entity_type_id");
            if (!isset($entity_id)) {
              return $this->subformState->setError($this->subform["entity_type_$entity_type_id"], $this->t('Please enter a specific entity to target.'));
            }
            else if (!$this->backgroundImage->getTargetEntity($this->type, "$entity_type_id:$entity_id")) {
              return $this->subformState->setError($this->subform["entity_type_$entity_type_id"], $this->t('Please enter a valid entity to target.'));
            }
          }
        }
        else if ($this->type === BackgroundImageInterface::TYPE_ENTITY_BUNDLE && ($target = $this->getSubformValue('entity_bundle'))) {
          return $this->subformState->setError($this->subform['view'], $this->t('Please select a valid entity bundle.'));
        }
        else if ($this->type === BackgroundImageInterface::TYPE_PATH && ($target = $this->getSubformValue('path'))) {
          return $this->subformState->setError($this->subform['path'], $this->t('Please enter a valid path.'));
        }
        else if ($this->type === BackgroundImageInterface::TYPE_ROUTE && ($target = $this->getSubformValue('route'))) {
          return $this->subformState->setError($this->subform['route'], $this->t('Please enter a valid route.'));
        }
        else if ($this->type === BackgroundImageInterface::TYPE_VIEW && ($target = $this->getSubformValue('view'))) {
          return $this->subformState->setError($this->subform['view'], $this->t('Please select a valid view.'));
        }
      }
    }
  }
